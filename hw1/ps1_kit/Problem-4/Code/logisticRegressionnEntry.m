...
path_test = "/home/snazyman/machine_learning/machine_learning/hw1/ps1_kit/Problem-4/Spambase/CrossValidation/Fold%d/cv-test.txt";
lambda = [10^-7 10^-6 10^-5 10^-4 10^-3 10^-2 10^-1 1];
for k = 1:length(lambda)
    for i = 1:5
        ...
        % Read hold out(test) fold
        testFold = sprintf(path_test, i);
        testFd = fopen(testFold);
        numExamples = 50;
        for v = 1:numExamples
            C_test(v) = textscan(testFd, '%f', 58);
        end
        fclose(testFd);
        ...
        % estimate parameter vector with logistic regression
        [w, b] = LogisticRegressionL2(traindata, trainlabels, lambda(k));
    
        % estimate y_hat
        y_hat = sign(testdata * w' + b);
        
        % calculate classification error on hold out set (test set)
        err_fold(i) = classification_error(y_hat, testlabels);
    end
    err(k) = mean(err_fold(:));
end
...
figure(3);
plot(lambda, err)
title('L2-Logistic Regression classification error as a function of lambda with cross-validation');
xlabel('lambda');
ylabel('classification error');
set(gca, 'Xscale', 'log')
print -deps l2logreg-crossval
